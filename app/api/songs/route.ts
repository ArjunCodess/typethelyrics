...
export async function POST(request: Request) {
  try {
    ...
    const { songDetails, spotifyUrl, userId } = await request.json();
    
    if (!songDetails || !spotifyUrl) {
      return NextResponse.json({ error: 'Missing song details' }, { status: 400 });
    }
